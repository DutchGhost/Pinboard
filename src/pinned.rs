...
pub trait IntoPin<T, S> {

    fn into_pin(this: T) -> Pin<S>;
}

impl <T> IntoPin<T, Self> for Arc<T> {

    fn into_pin(this: T) -> Pin<Self> {
        Arc::pin(this)
    }
}

impl <'a, T: Unpin> IntoPin<Self, &'a T> for &'a mut Arc<T> {

    fn into_pin(this: Self) -> Pin<&'a T> {
        Pin::new(&*this)
    }
}

impl <T> IntoPin<T, Self> for Box<T> {
    fn into_pin(this: T) -> Pin<Self> {
        Box::pin(this)
    }
}

impl <'a, T: 'a + ?Sized + Unpin> IntoPin<Self, &'a T> for &'a Box<T> {
    fn into_pin(this: Self) -> Pin<&'a T> {
        Pin::new(&*this)
    }
}

impl <'a, T: 'a + ?Sized + Unpin> IntoPin<Self, &'a mut T> for &'a mut Box<T> {
    fn into_pin(this: Self) -> Pin<&'a mut T> {
        Pin::new(&mut *this)
    }
}

impl <'a, T: 'a + ?Sized + Unpin> IntoPin<Self, &'a T> for &'a mut Box<T> {
    fn into_pin(this: Self) -> Pin<&'a T> {
        Pin::new(&mut *this)
    }
}
...
//     fn into_pin(this: T) -> Pin<Self::Target> {
//         Box::pin(this)
//     }
// }

// impl <'a, T: Unpin + ?Sized> IntoPin<Self> for &'a Box<T> {
//     type Target = &'a T;

//     fn into_pin(this: &'a Box<T>) -> Pin<Self::Target> {
//         Pin::new(&*this)
//     }
// }

// impl <'a, T: Unpin + ?Sized> IntoPin<Self> for &'a mut Box<T> {
//     type Target = &'a T;

//     fn into_pin(this: &'a mut Box<T>) -> Pin<Self::Target> {
//         Pin::new(&*this)
//     }
// }
...
//     fn into_pinned<'a, T: 'a, P: IntoPin<P, Target = &'a T>>(p: P) {
//         let x: Pin<&'a T> = P::into_pin(p);
//     }

//     into_pinned(&mut Arc::new(10));
// }
// use std::borrow::Cow;
